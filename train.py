...
def accuracy_CW(model, dataset_loader, c=1e-4, kappa=0, device=None):
    attack = torchattacks.CW(model, c=c, kappa=kappa)
    return evaluate_model(model, dataset_loader, attack, f"CW (c={c})", device)

def accuracy_Square(model, dataset_loader, eps, device):
...
def accuracy_AutoAttack(model, dataset_loader, num_classes, eps, device):
...
if __name__ == '__main__':
    ...
    if args.dataset == "lisa":
        ...
        train_loader, test_loader, train_eval_loader, num_classes = bstl_loaders(args.env, 512, args.normalize) 
    ...
    for eps in all_eps:
        acc, prec, rec, f1 = accuracy_FGSM(model, test_loader, eps, device)
    
    for eps in all_eps:
        acc, prec, rec, f1 = accuracy_PGD(model, test_loader, eps, device)

    for eps in all_eps:
        acc, prec, rec, f1 = accuracy_MIM(model, test_loader, eps, device)

    #for eps in all_eps:
    #    acc, prec, rec, f1 = accuracy_Square(model, test_loader, eps, device)
    
    for eps in all_eps:
        acc, prec, rec, f1 = accuracy_AutoAttack(model, test_loader, 4, eps, device)
